...
def run():
    ...
    c.find('FromDIP').findOverload('(const wxSize &sz, const wxWindow *w)').ignore()
    c.find('ToDIP').findOverload('(const wxSize &sz, const wxWindow *w)').ignore()

    # c.find('ToDIP').findOverload(') const').ignore()
